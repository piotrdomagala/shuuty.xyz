...
export function validateStoreMediaProvenance(
  ...
  let ownerAuthorizedLikenessCount = 0;
  let notApplicableLikenessCount = 0;
  for (const [index, asset] of mediaAssets.entries()) {
    ...
    requireValue(
      assetId !== "missing" && !assetIds.has(assetId),
    ...
    const fileName =
      typeof asset.fileName === "string" && asset.fileName
        ...
        : "missing";
    requireValue(
      fileName !== "missing" && !fileNames.has(fileName),
    ...
    requireValue(
      asset.format === "webp",
      `${assetId} must be recorded as WebP`,
    );
    requireValue(
    ...
    requireValue(
      Array.isArray(asset.locales) &&
        ...
        asset.locales.every((locale) =>
          STORE_METADATA_LOCALES.includes(locale),
        ),
    ...
    if (likeness.releaseStatus === "owner-authorized") {
      ownerAuthorizedLikenessCount += 1;
    }
    if (likeness.releaseStatus === "not-applicable") {
      notApplicableLikenessCount += 1;
    }
  }
