...
/**
 * Long dashes anywhere; `**` must be paired where it renders as bold and is
 * not allowed at all in plain-text fields (titles, headings, labels).
 *
 * @param {string} where
 * @param {unknown} value
 ...
 * @returns {string[]}
 */
function textProblems(where, value, { plain = false } = {}) {
  const problems = [];
  for (const text of collectStrings(value)) {
    ...
    if (plain && text.includes('**')) {
      problems.push(`${where}: ** is shown literally in plain text: ${text}`);
      continue;
    }
    try {
      textSegments(text);
...
export function guideRegistryProblems(registry) {
  const problems = [...textProblems('content/guides.json', registry, { plain: true })];
...
export function guideArticleProblems(name, article) {
  ...
  body.forEach((block, index) => {
    if (!BLOCK_TYPES.has(block?.type)) {
      ...
      problems.push(...textProblems(`${name}: ${block.type} block ${index}`, block.text, { plain: true }));
    }
  });
